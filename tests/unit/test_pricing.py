...
class TestCostCalculator:
    ...
    def test_calculate_cost_gpt4(self):
        ...
        cost = calculator.calculate_cost(
            provider="openai", model="gpt-4", prompt_tokens=1000, completion_tokens=500
        )
    ...
    def test_calculate_cost_gpt35(self):
        """测试 GPT-3.5 成本计算"""
        cost = calculator.calculate_cost(
            provider="openai", model="gpt-3.5-turbo", prompt_tokens=2000, completion_tokens=1000
        )
    ...
    def test_calculate_cost_unknown_model(self):
        """测试未知模型应返回 0 成本"""
        cost = calculator.calculate_cost(
            provider="openai", model="unknown-model", prompt_tokens=1000, completion_tokens=500
        )
    ...
    def test_calculate_cost_unknown_provider(self):
        """测试未知提供商应返回 0 成本"""
        cost = calculator.calculate_cost(
            provider="unknown-provider", model="gpt-4", prompt_tokens=1000, completion_tokens=500
        )
    ...
    def test_custom_pricing(self):
        """测试自定义定价配置"""
        custom_pricing = {"custom": {"custom-model": {"input": 0.01, "output": 0.02}}}

        calc = CostCalculator(custom_pricing=custom_pricing)
        cost = calc.calculate_cost(
            provider="custom", model="custom-model", prompt_tokens=1000, completion_tokens=1000
        )
