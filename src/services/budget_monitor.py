...
class BudgetMonitor:
    ...
    async def _check_single_budget(self, db: Session, budget: Budget):
        ...
        try:
            ...
            error(logger, "Failed to check budget", budget_id=budget.id, error=str(e))
    ...
    async def _send_notification(self, budget: Budget, trigger: dict):
        ...
        try:
            ...
            info(logger, "Notification sent", budget_id=budget.id, results=results)

        except Exception as e:
            error(logger, "Failed to send notification", budget_id=budget.id, error=str(e))
