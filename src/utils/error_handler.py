...
class NotFoundError(APIException):
    ...
    def __init__(self, message: str = "资源不存在", resource: str = ""):
        super().__init__(
            message=message, status_code=404, error_code="NOT_FOUND", details={"resource": resource}
        )
...
def create_error_response(
    exception: Exception, request_id: str, include_traceback: bool = False
) -> Dict[str, Any]:
    """
    创建标准错误响应格式
...
class ErrorHandlerMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            error_response = create_error_response(e, get_request_id(), self.debug)
            return JSONResponse(
                status_code=500, content=error_response, headers={"X-Request-ID": get_request_id()}
            )
