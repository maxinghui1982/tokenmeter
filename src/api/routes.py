...
async def get_summary(
    ...
    ).filter(UsageRecord.timestamp >= since).first()
    ...
    ).filter(UsageRecord.timestamp >= since).group_by(UsageRecord.provider).all()
    ...
    model_stats = db.query(
        ...
        func.sum(UsageRecord.total_tokens).label("tokens"),
        func.sum(UsageRecord.cost_total).label("cost")
    ).filter(UsageRecord.timestamp >= since).group_by(UsageRecord.model).all()
...
async def get_project_stats(
    ...
    stats = db.query(
        ...
        func.sum(UsageRecord.total_tokens).label("tokens"),
        func.sum(UsageRecord.cost_total).label("cost")
    ).filter(UsageRecord.timestamp >= since).group_by(UsageRecord.project).order_by(
        func.sum(UsageRecord.cost_total).desc()
    ).all()
...
async def get_team_stats(
    ...
    stats = db.query(
        ...
        func.sum(UsageRecord.total_tokens).label("tokens"),
        func.sum(UsageRecord.cost_total).label("cost")
    ).filter(UsageRecord.timestamp >= since).group_by(UsageRecord.team).order_by(
        func.sum(UsageRecord.cost_total).desc()
    ).all()
...
async def get_daily_stats(
    ...
    stats = query.group_by(func.date(UsageRecord.timestamp)).order_by(
