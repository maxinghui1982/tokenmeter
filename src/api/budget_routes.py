...
from ..database.budget_models import (
    Budget,
    BudgetAlert,
    BudgetCalculator,
    BudgetPeriod,
...
async def get_budget_alerts(
    budget_id: int, limit: int = Query(100, ge=1, le=1000), db: Session = Depends(get_db)
):
    """获取预算告警历史"""
    alerts = (
...
async def get_budget_summary(db: Session = Depends(get_db)):
    ...
    total_alerts = (
        db.query(BudgetAlert)
        .filter(BudgetAlert.created_at >= datetime.utcnow() - timedelta(days=7))
        .count()
    )
