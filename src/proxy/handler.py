    ...
    def _init_providers(self, configs: Dict):
        ...
        for provider_name, default_config in default_configs.items():
            ...
            try:
                ...
                error(
                    logger, f"Failed to initialize provider", provider=provider_name, error=str(e)
                )

    async def proxy_request(self, provider: str, request: Request, path: str = "") -> Response:
        """
        代理请求并记录用量
        ...
        if not provider_instance:
            raise HTTPException(status_code=400, detail=f"Unknown or disabled provider: {provider}")
        ...
        try:
            ...
            return Response(
                content=content, status_code=response.status_code, headers=dict(response.headers)
            )

        except httpx.RequestError as e:
            error(logger, f"Proxy request failed", provider=provider, error=str(e))
